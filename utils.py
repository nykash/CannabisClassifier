...
class GridSearchHyperParameters(object):
    def __init__(self, range_steps):
        self.range_steps = range_steps
        self.params = []
        for key in range_steps:
            start, stop, n = range_steps[key]
            self.params.append(np.linspace(start, stop, n).tolist())

    def fivefold_crossvalid_solve(self, x_train, y_train, model_fn):
        metrics = []

        skf = StratifiedKFold(n_splits=5)

        for i, params in enumerate(zip(*self.params)):
            model = model_fn(*params)

            sum_acc = 0

            for j, (train_index, test_index) in enumerate(skf.split(x_train, y_train)):
                model_cln = clone(model)
                ...
                sum_acc += (model_cln.predict(fold_x_test) == fold_y_test).mean()

                print("\r", f"currently processing param {i}/{len(list(zip(*self.params)))}, fold {j}/{5}", end="")

            metrics.append([params, sum_acc/5])
        print()
...
from sklearn.metrics import confusion_matrix, roc_auc_score, roc_curve, auc, accuracy_score, \
    precision_recall_fscore_support, jaccard_score, RocCurveDisplay
...
def digitize_rdm(rdm_raw, n_bins=10):
    ...
    rdm_vec_digitized = np.digitize(np.ravel(rdm_raw), bins=rdm_bins) * (
                100 // n_bins)  # Compute the vectorized digitized value
...
class ClassifierPipeline(object):
    ...
    def fit(self, X, y, numfolds):
        ...
        for c, (classifier_name, classifier) in enumerate(self.classifier_dict.items()):
            ...
            for k, (train, test) in enumerate(cv.split(X, y)):
                ...
                viz = RocCurveDisplay.from_estimator(classifier_clone, X[test], y[test], name=f'ROC fold {k + 1}', alpha=0.3, lw=1,
                                     ax=ax[c])
    ...
    def ensemble_prediction(self):
        ...
        self.probas[:, c + 1] = np.mean(self.probas[:, 0:len(self.classifier_dict)],
                                        axis=1)  # average all probabilities togehter
        predictions = (self.probas > .5).astype(int).astype(int)  # class 1 predictions

        labels_ordered = self.y[
            self.test_ixs[classifier_name]]  # training set indices should be the same across classifiers
    ...
    def predictions_plot(self, prediction_matrix, y_tick_labels, names):
        ...
        sns.heatmap(prediction_matrix.T[:, sorted_ind], yticklabels=y_tick_labels,
                    xticklabels=np.array(names)[training == 1][sorted_ind], cmap='Blues', linewidths=.25,
